...
const Hero = () => {
  ...
  const [currentHeroImage, setCurrentHeroImage] = useState(null); // ❌ don't initialize directly

  useEffect(() => {
    const lastImageIndex = localStorage.getItem("lastHeroImageIndex");
    let newIndex;

    // Ensure different image from last shown one
    do {
      newIndex = Math.floor(Math.random() * heroImages.length);
    } while (
      heroImages.length > 1 &&
      newIndex.toString() === lastImageIndex
    );

    // Save new index to localStorage
    localStorage.setItem("lastHeroImageIndex", newIndex.toString());

    // Set new hero image
    setCurrentHeroImage(heroImages[newIndex]);
  }, []);
    ...
    >
      <div className="container realtive" ref={parallaxRef}>
        <div className="relative z-1 max-w-[62rem] mx-auto text-center mb-[3.875rem] md:mb-20 lg:mb-[6.25rem]">
          ...
          <h1 className="h2">
            <span>
              An Innovative and Economical Hearing Solution for Crystal Clear
              Sound
            </span>
          </h1>
          <p className="body-1 max-w-3xl mx-auto mt-3 mb-6 text-n-4 lg:mb-8">
            HearMate provides an economical and convenient over-the-counter
            hearing solution by converting your earphones and headphones into
